...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const iconIndex = childrenArray.findIndex(child => React.isValidElement(child) && (child.type as any).displayName?.includes('Icon'));
    
    const textChildren = childrenArray.filter((_, i) => i !== iconIndex);
    const iconChild = childrenArray[iconIndex];
